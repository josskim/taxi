...
class MyHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif self.path == "/profileImages":
            # 등록된 사진 목록을 JSON으로 반환 (파일명: /image/<driverId>.png)
            images = {}
            try:
                for filename in os.listdir(IMAGE_DIR):
                    if filename.startswith('.'):
                        continue
                    driver_id, ext = os.path.splitext(filename)
                    # 서버 URL 경로 기준으로
                    images[driver_id] = f"/{IMAGE_DIR}/{filename}"
                self.send_response(200)
                self.send_header("Content-type", "application/json; charset=utf-8")
                self.end_headers()
    ...
    def do_POST(self):
        if self.path == "/uploadProfileImage":
            # multipart/form-data 처리
            ctype, pdict = cgi.parse_header(self.headers.get('Content-Type'))
            if ctype != 'multipart/form-data':
                self.send_response(400)
                self.end_headers()
                self.wfile.write("잘못된 Content-Type".encode('utf-8'))
                return
            ...
            try:
                ...
                driver_id = fields.get('id')
                files = fields.get('image')
                if not driver_id or not files:
                    raise ValueError("필수 항목 누락")
                driver_id = driver_id[0]
                filebytes = files[0]
                # 파일 저장: image/<driver_id>.png (단순히 png 확장자로 저장)
                if not os.path.exists(IMAGE_DIR):
                    os.makedirs(IMAGE_DIR)
                filepath = os.path.join(IMAGE_DIR, f"{driver_id}.png")
